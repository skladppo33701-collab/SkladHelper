...
class AssignmentParser {
  ...
  /// Aligns with the Assignment model requirements for name, type, and createdAt.
  static Assignment? parseText(String content) {
    ...
    try {
      ...
      return Assignment(
        id: const Uuid().v4(),
        name:
            "Заказ $orderId - $clientName", // Combined into name as model fields are restricted
        type: "Warehouse Task",
        status: AssignmentStatus
            .inProgress, // Corrected: Using inProgress instead of active/pending
  ...
  /// Extracts data from Excel/Spreadsheet bytes.
  /// This provides the 'parseExcelBytes' method required by the InventoryPage.
  static Assignment parseExcelBytes(Uint8List bytes, String fileName) {
    final List<AssignmentItem> items = [];
    String docTitle = fileName.split('.').first;
    ...
    for (var table in decoder.tables.keys) {
      ...
      for (var row in rows) {
        ...
        if (headerFound && colSku < row.length && colName < row.length) {
          ...
          if (qty > 0) {
            items.add(AssignmentItem(name: name, code: sku, requiredQty: qty));
          }
        }
      }
    }
    ...
    if (items.isEmpty) {
      throw Exception("Товары не найдены в файле");
    }
